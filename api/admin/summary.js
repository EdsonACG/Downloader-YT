export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(`${backendUrl.replace(/\/$/, '')}/api/admin/summary`, {
    ...
    return res.status(502).json({ error: 'Falha no painel admin.', detail: String(error?.message || error) });
  }
}
